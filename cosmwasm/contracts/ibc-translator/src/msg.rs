...
#[cw_serde]
pub enum ExecuteMsg {
    /// Submit a VAA to complete a deltaswap payload3 token bridge transfer.
    /// This function will:
    /// 1. complete the deltaswap token bridge transfer.
    ...
    /// Convert bank tokens into the equivalent (locked) cw20 tokens and trigger a deltaswap token bridge transfer.
    /// This function will:
    ...
    /// 4. Cross-call into the deltaswap token bridge to initiate a cross-chain transfer with a gateway transfer payload.
    ...
    /// 2. Burn the bank tokens using the token factory.
    /// 3. Unlock the equivalent cw20 tokens.
    /// 4. Cross-call into the deltaswap token bridge to initiate a cross-chain transfer with a gateway transfer-with-payload payload.
