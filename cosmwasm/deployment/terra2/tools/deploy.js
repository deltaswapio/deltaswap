...
// Generated using
// `phylaxd template ibc-receiver-update-channel-chain --channel-id channel-0 --chain-id 7077 --target-chain-id 32 > terra2.prototxt`
// `phylaxd admin governance-vaa-verify terra2.prototxt`
const DELTASWAP_IBC_WHITELIST_VAA =
...
addresses["deltaswap_ibc.wasm"] = await instantiate(
  "deltaswap_ibc.wasm",
  {
    gov_chain: govChain,
    gov_address: Buffer.from(govAddress, "hex").toString("base64"),
...
// submit deltachain channel ID whitelist to the deltaswap_ibc contract
const ibc_whitelist_tx = await wallet.createAndSignTx({
  msgs: [
    new MsgExecuteContract(
      wallet.key.accAddress,
      addresses["deltaswap_ibc.wasm"],
      {
        submit_update_channel_chain: {
          vaa: Buffer.from(DELTASWAP_IBC_WHITELIST_VAA, "hex").toString(
            "base64"
          ),
        },
      },
...
console.log("updated deltaswap_ibc channel whitelist", ibc_whitelist_res.txhash);
