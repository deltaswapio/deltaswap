...
export async function getDeltaswapRelayerInfo(
  ...
  const deliveryLog = getDeltaswapLog(
    receipt,
    bridgeAddress,
  ...
  if (type === RelayerPayloadId.Redelivery) {
    ...
    const transactionHash = await getRelayerTransactionHashFromWormscan(
        ...
        redeliveryInstruction.deliveryVaaKey.sequence.toNumber(),
        {
          ...
          deltaswapRelayerAddress: infoRequest?.deltaswapRelayerAddresses?.get(
              originalSourceChainName
          ),
        }
    );

    return getDeltaswapRelayerInfo(
        originalSourceChainName,
        transactionHash,
        modifiedInfoRequest
    );
            ...
            if (messageKey.keyType === 1) {
              ...
              try {
                if (
                  ...
                  tokenBridgeLog = getDeltaswapLog(
                      receipt,
  ...
  const targetChainDeliveries = await getDeltaswapRelayerInfoByHash(
      deliveryHash,
    targetChain,
    sourceChain,
      sourceSequence.toNumber(),
  ...
  const stringified = stringifyDeltaswapRelayerInfo(result);
