...
// ParseAbiABI parses the ABI
func ParseAbiABI() (*abi.ABI, error) {
	parsed, err := abi.JSON(strings.NewReader(AbiABI))
	if err != nil {
		return nil, err
	}
...
func (_Abi *AbiFilterer) TryParseLog(log types.Log) (eventName string, event interface{}, ok bool, err error) {
	eventName, ok, err = _Abi.contract.LogEventName(log)
	if err != nil || !ok {
	...
	switch eventName {
	case "AdminChanged":
		event, err = _Abi.ParseAdminChanged(log)
	case "BeaconUpgraded":
	...
	case "Upgraded":
		event, err = _Abi.ParseUpgraded(log)
	}
	if err != nil {
		return "", nil, false, err
	}
