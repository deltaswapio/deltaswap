...
Responsibility: Observe finalized `publishMessage` event emissions from the Deltaswap Core account on the NEAR blockchain.
...
* *TxProcessor*: Processes `transactionProcessingJob`, going through all receipt outcomes, searching for Deltaswap messages, and checking that they have been finalized. If there are Deltaswap messages in any receipts from this transaction but those receipts are not in finalized blocks, the `transactionProcessingJob` will be put in the back of the queue.
* *ObsvReqProcessor*: Process observation requests. An observation request is a way of kindly asking the Phylax to go back in time and look at a particular transaction and try to identify deltaswap events in it. Observation requests are received from other Phylaxs or injected through the admin API. Eventhough they are signed, they should not be trusted.
...
* log_msg_type (enum)
	...
	* deltaswap_event: A Deltaswap event is being processed
	* deltaswap_event_success: A Deltaswap event has been successfully processed
...
* error_type (enum)
	...
	* malformed_deltaswap_event: The deltaswap log emission is malformed. This should never happen. If it does, that'd be indicative of a big problem.
	* startup_fail: Something went wrong during watcher startup.


## Assumptions
* We assume that transactions containing Deltaswap messages are finalized on the NEAR blockchain in `initialTxProcDelay ^ (txProcRetry+2)` time after the block containing the start of the transaction has been observed. Otherwise they will be missed. Strong network congestion or gaps/delays in block production could violate this.
...
RUN git clone https://github.com/deltaswap-foundation/deltaswap.git

WORKDIR /deltaswap/ethereum
RUN npm ci
RUN npm run build

WORKDIR /deltaswap/sdk/js
RUN npm ci
RUN npm run build-all

WORKDIR /deltaswap/near
RUN npm ci
