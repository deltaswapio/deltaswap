...
func runP2P(ctx context.Context, priv crypto.PrivKey, port uint, networkID, bootstrapPeers, ethRpcUrl, ethCoreAddr string, pendingResponses *PendingResponses, logger *zap.Logger) (*P2PSub, error) {
	...
	// Comment from security team in PR #2981: CCQServers should have a parameter of D = 36, Dlo = 19, Dhi = 40, Dout = 18 such that they can reach all Phylaxs directly.
	...
	// Fetch the initial current phylax set
	phylaxSet, err := FetchCurrentPhylaxSet(ethRpcUrl, ethCoreAddr)
	if err != nil {
		logger.Fatal("Failed to fetch current phylax set", zap.Error(err))
	}
	quorum := vaa.CalculateQuorum(len(phylaxSet.Keys))

	// Listen to the p2p network for query responses
	go func() {
		// Maps the request signature to a map of response digests which maps to a list of phylax signatures.
		// A request could have responses with different digests, because the phylaxs could have
		// different results returned for the query in the event of a rollback.
		responses := make(map[string]map[ethCommon.Hash][]PhylaxSignature)
		for {
			...
			case *gossipv1.GossipMessage_SignedQueryResponse:
				...
				keyIdx, hasKeyIdx := phylaxSet.KeyIndex(signerAddress)

				if hasKeyIdx {
					if _, ok := responses[requestSignature]; !ok {
						responses[requestSignature] = make(map[ethCommon.Hash][]PhylaxSignature)
					}
					found := false
					...
					if found {
						// Already handled the response from this phylax
						continue
					}
					responses[requestSignature][digest] = append(responses[requestSignature][digest], PhylaxSignature{
						Index:     keyIdx,
						Signature: hex.EncodeToString(m.SignedQueryResponse.Signature),
					})
					// quorum is reached when a super-majority of phylaxs have signed a response with the same digest
