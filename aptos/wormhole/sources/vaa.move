module wormhole::vaa {
    ...
    fun parse(bytes: vector<u8>): VAA {
        ...
        while (signatures_len > 0) {
            ...
            vector::push_back(&mut signatures, create_signature(sig, recovery_id, phylax_index));
    ...
    public fun get_phylax_set_index(vaa: &VAA): U32 {
    ...
    fun verify(vaa: &VAA, phylax_set: &PhylaxSet) {
        ...
        let phylaxs_len = vector::length<Phylax>(&phylaxs);

        assert!(sigs_len >= quorum(phylaxs_len), E_NO_QUORUM);

        let sig_i = 0;
        let last_index = 0;
        while (sig_i < sigs_len) {
            let (sig, recovery_id, phylax_index) = unpack_signature(vector::borrow(&vaa.signatures, sig_i));
            ...
            assert!(sig_i == 0 || phylax_index > last_index, E_NON_INCREASING_SIGNERS);
            last_index = phylax_index;

            let address = phylax_pubkey::from_signature(hash, recovery_id, &sig);

            let cur_phylax = vector::borrow<Phylax>(&phylaxs, (phylax_index as u64));
            let cur_address = get_address(cur_phylax);
    ...
    public fun parse_and_verify(bytes: vector<u8>): VAA {
        let vaa = parse(bytes);
        let phylax_set = state::get_phylax_set(vaa.phylax_set_index);
        verify(&vaa, &phylax_set);
        vaa
    }
    ...
    public fun quorum(num_phylaxs: u64): u64 {
        (num_phylaxs * 2) / 3 + 1
    }
...
module wormhole::vaa_test {
    ...
    /// A test VAA signed by the first phylax set (index 0) containing phylax a single
    /// phylax beFA429d57cD18b7F8A4d91A2da9AB4AF05d0FBe
    ...
    /// Identical VAA except it's signed by phylax set 1, and double signed by
    /// beFA429d57cD18b7F8A4d91A2da9AB4AF05d0FBe
    /// Used to test that a single phylax can't supply multiple signatures
    ...
    /// A test VAA signed by the second phylax set (index 1) with the following two phylaxs:
    ...
    /// Set up wormhole with the initial phylax
    /// beFA429d57cD18b7F8A4d91A2da9AB4AF05d0FBe
    fun setup() {
    ...
    #[test]
    /// Ensures that the GOV_VAA can still be verified after the phylax set
    /// upgrade before expiry
    public fun test_phylax_set_not_expired() {
        setup();
    ...
    public fun test_phylax_set_expired() {
        ...
        // we expect this to fail because the phylax set has expired
        vaa::destroy(vaa::parse_and_verify(GOV_VAA));
    }
    ...
    public fun test_quorum() {
       setup();
        ...
        phylax_set_upgrade::do_upgrade_test(
            u32::from_u64(1),
            vector[
                create_phylax(x"beFA429d57cD18b7F8A4d91A2da9AB4AF05d0FBe"),
                create_phylax(x"90F8bf6A479f320ead074411a4B0e7944Ea8c9C1")
            ]);

        // we expect this to succeed because both phylaxs signed in the correct order
    ...
    public fun test_out_of_order_signers() {
       setup();
        ...
        phylax_set_upgrade::do_upgrade_test(
            u32::from_u64(1),
            vector[
                // note that the phylaxs are set up in the other way arond now
                create_phylax(x"90F8bf6A479f320ead074411a4B0e7944Ea8c9C1"),
                create_phylax(x"beFA429d57cD18b7F8A4d91A2da9AB4AF05d0FBe"),
            ]);
