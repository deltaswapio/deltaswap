...
set last_run_file "ts-scripts/relayer/output/$ENV/deployDeltaswapRelayer/lastrun.json"
...
for chain in $chain_ids
    ...
    set proxy_address (jq --raw-output ".deltaswapRelayerProxies[] | select(.chainId == $chain) | .address" $last_run_file)
    ...
    if test $chain -eq 14
        forge verify-contract $proxy_address contracts/relayer/create2Factory/Create2Factory.sol:SimpleProxy --watch --constructor-args $init_contract_address --show-standard-json-input > DeltaswapRelayerProxy.compiler-input.json
        forge verify-contract $implementation_address DeltaswapRelayer --watch --constructor-args $deltaswap_address --show-standard-json-input > DeltaswapRelayerImplementation.compiler-input.json
        ...
        echo "- $proxy_address: DeltaswapRelayerProxy.compiler-input.json"
    else
